...
import { BAND_TRANSITION_EASE } from "@/lib/brand-transition";
...
export function BandExploreRail() {
  ...
  const slideOffset = isRight ? 28 : -28;
  const edgeTransition = prefersReducedMotion
    ? { duration: 0 }
    : { duration: 0.35, ease: BAND_TRANSITION_EASE };
      ...
      <AnimatePresence mode="sync">
        {navVisible ? (
          <motion.div
            ...
            className={`explore-rail-edge pointer-events-none absolute inset-y-0 z-[45] ${
              config.edgeClass
      ...
      <AnimatePresence mode="wait">
        {navVisible ? (
          <motion.aside
            ...
            transition={edgeTransition}
            className={`explore-rail-tab-host pointer-events-none fixed top-0 z-[46] w-0 ${
              isRight ? "right-0" : "left-0"
            }`}
          >
            <motion.button
              ...
              className={`explore-rail-tab pointer-events-auto absolute top-auto z-10 flex max-lg:translate-none flex-col items-center border-2 max-lg:bottom-10 sm:tall:max-lg:bottom-12 lg:bottom-auto lg:top-1/2 lg:-translate-y-1/2 ${targetMeta.accentBorder} ${targetMeta.bandBg} ${targetMeta.inactiveShadow} can-hover:hover:shadow-[3px_3px_0_rgba(42,31,20,0.2)] ${
                isRight
